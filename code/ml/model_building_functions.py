...
def make_figure_score(y_test_prob, y_test, label):
    ...
    # score normalization
    scores_unfixed = score_normalization_batch(y_test_prob)
    scores_fixed = []
    ...
    #plt.show()
    plot_img_name_prefix = os.path.split(label)
    plot_img_name = plot_img_name_prefix[0] + "/score_plot.jpg"
    print plot_img_name
    plt.savefig(plot_img_name)
...
def make_gains_chart(y_proba, y_test, step, label):
    ...
    ##plt.show()
    plot_img_name_prefix = os.path.split(label)
    plot_img_name = plot_img_name_prefix[0] + "/ks_plot.jpg"
    print plot_img_name
    plt.savefig(plot_img_name)
...
def calc_model_metrics(model, X, y):
    ...
    return test_fpr, test_tpr, test_roc_auc
                             ...
                             Original Message                             
Sender: 张永杰<zhangyongjie@hbc315.com>
Recipient: 程世庆<chengshiqing@hbc315.com>
Date: Saturday, Mar 26, 2016 15:43
Subject: Re:
...
------------------ Original ------------------
From:  "张永杰"<zhangyongjie@hbc315.com>;
Date:  Sat, Mar 26, 2016 03:32 PM
To:  "程世庆"<chengshiqing@hbc315.com>;
Subject:  
...
def make_figure_score(y_test_prob, y_test, label):
    # load score base file
    f = open(setting.INPUT_DIR + 'scores_base.pkl', 'rb')
    scores_base ＝ pickle.dump(f)
    f.close()
    ...
    scores_fixed = []
    for score in scores_unfixed:
        score_fixed = score_normalization(300, 900, stats.percentileofscore(scores_base, score, kind='mean') / 100.0)
        scores_fixed.append(score_fixed)
